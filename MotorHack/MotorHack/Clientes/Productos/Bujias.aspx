<%@ Page Title="" Language="C#" MasterPageFile="~/Clientes/Cliente.Master" AutoEventWireup="true" CodeBehind="Bujias.aspx.cs" Inherits="MotorHack.Clientes.Productos.Bujias" %>
                ...
                <div class="col-sm-6 col-md-4">
                    <div class="thumbnail">
                        <div class="caption">
                            ...
                            Precio:<label><%#DataBinder.Eval(Container.DataItem, "precio_venta")%></label>Stock: <label><%#DataBinder.Eval(Container.DataItem, "existencia")%>  </label>
                            ...
                            <input type="button" class="btn btn-primary" value="Agregar a carrito" onclick="agregarCarrito('<%#DataBinder.Eval(Container.DataItem, "precio_venta")%>    ','<%#DataBinder.Eval(Container.DataItem, "nombre")%>    ', 'txtCantidad_<%#DataBinder.Eval(Container.DataItem, "id_producto")%> ','<%#DataBinder.Eval(Container.DataItem, "descripion")%>   ', '<%#DataBinder.Eval(Container.DataItem, "id_producto")%>');" /></div>
                       </div>
                </div>
            </ItemTemplate>
        </asp:Repeater>
    ...
    <script type="text/javascript">
        function agregarCarrito(precio_venta, p_nombre, p_cantidad, p_descripcion,p_id_producto) {
            ...
            jQuery.ajax({
                type        : "GET",
                url         : "../acciones.aspx",
                data        : { accion : "agregar_bujias", id_producto: p_id_producto,nombre:p_nombre, cantidad: t_cantidad, precio:precio_venta},
